...
var ZRender = function() {
  ...
  ZRender2.prototype.pathToImage = function(e2, dpr2) {
...
var PathProxy = function() {
  ...
  PathProxy2.prototype._dashedLineTo = function(x1, y1) {
    ...
    var dx = x1 - x0;
    var dy = y1 - y0;
    var dist3 = mathSqrt2(dx * dx + dy * dy);
    var x = x0;
    var y = y0;
  ...
  for (i = 0, j = 0; i < len2; ) {
    ...
    switch (cmd) {
      ...
      case A:
        var x = m2[4];
        var y = m2[5];
        var sx = mathSqrt3(m2[0] * m2[0] + m2[1] * m2[1]);
        var sy = mathSqrt3(m2[2] * m2[2] + m2[3] * m2[3]);
...
function processArc(x1, y1, x2, y2, fa, fs, rx, ry, psiDeg, cmd, path) {
  ...
  var f = (fa === fs ? -1 : 1) * mathSqrt4((rx * rx * (ry * ry) - rx * rx * (yp * yp) - ry * ry * (xp * xp)) / (rx * rx * (yp * yp) + ry * ry * (xp * xp))) || 0;
  var cxp = f * rx * yp / ry;
  var cyp = f * -ry * xp / rx;
...
function computeCornerTangents(x0, y0, x1, y1, radius, cr, clockwise) {
  var x01 = x0 - x1;
  var y01 = y0 - y1;
  var lo = (clockwise ? cr : -cr) / mathSqrt5(x01 * x01 + y01 * y01);
  var ox = lo * y01;
  var oy = -lo * x01;
  var x11 = x0 + ox;
  var y11 = y0 + oy;
  ...
  var d = (dy < 0 ? -1 : 1) * mathSqrt5(mathMax4(0, r * r * d2 - s * s));
...
function buildPath2(ctx, shape) {
  ...
  if (startAngle === endAngle) {
    ...
    var tmpAngles2 = [startAngle, endAngle];
    normalizeArcAngles(tmpAngles2, !clockwise);
    arc2 = mathAbs2(tmpAngles2[0] - tmpAngles2[1]);
  }
  var x = shape.cx;
  var y = shape.cy;
  ...
  if (!(radius > e)) {
    ...
    ctx.moveTo(x + radius * mathCos4(startAngle), y + radius * mathSin4(startAngle));
    ctx.arc(x, y, radius, startAngle, endAngle, !clockwise);
    if (innerRadius > e) {
      ctx.moveTo(x + innerRadius * mathCos4(endAngle), y + innerRadius * mathSin4(endAngle));
      ctx.arc(x, y, innerRadius, endAngle, startAngle, clockwise);
    }
  } else {
    var halfRd = mathAbs2(radius - innerRadius) / 2;
    var cr = mathMin4(halfRd, cornerRadius);
    var icr = mathMin4(halfRd, innerCornerRadius);
    var cr0 = icr;
    var cr1 = cr;
    ...
    if (cr > e || icr > e) {
      ...
      if (arc2 < PI4) {
        ...
        if (it_1) {
          ...
          var x1 = xre - it_1[0];
          var y1 = yre - it_1[1];
          var a = 1 / mathSin4(mathACos((x0 * x1 + y0 * y1) / (mathSqrt5(x0 * x0 + y0 * y0) * mathSqrt5(x1 * x1 + y1 * y1))) / 2);
          var b = mathSqrt5(it_1[0] * it_1[0] + it_1[1] * it_1[1]);
          cr0 = mathMin4(icr, (innerRadius - b) / (a - 1));
          cr1 = mathMin4(cr, (radius - b) / (a + 1));
        }
      }
    }
    if (!(arc2 > e)) {
...
function smoothSpline(points4, isLoop) {
  ...
  for (var i = 0; i < segs; i++) {
    ...
    var w = pos - idx;
    var p0 = void 0;
    var p1 = points4[idx % len2];
    var p2 = void 0;
    var p3 = void 0;
    if (!isLoop) {
      p0 = points4[idx === 0 ? idx : idx - 1];
      p2 = points4[idx > len2 - 2 ? len2 - 1 : idx + 1];
      p3 = points4[idx > len2 - 3 ? len2 - 1 : idx + 2];
    } else {
      p0 = points4[(idx - 1 + len2) % len2];
      p2 = points4[(idx + 1) % len2];
      p3 = points4[(idx + 2) % len2];
    }
    var w2 = w * w;
    var w3 = w * w2;
    ret.push([
...
var DefaultDataProvider = function() {
  ...
  DefaultDataProvider2.internalField = function() {
    ...
    mountMethods = function(provider, data, source) {
      ...
      var methods3 = providerMethods[getMethodMapKey(sourceFormat, seriesLayoutBy)];
      if (true) {
        assert(methods3, "Invalide sourceFormat: " + sourceFormat);
      }
      extend(provider, methods3);
...
function createCanvasPattern(ctx, pattern, el) {
  ...
  if (isImageReady(image)) {
    ...
    if (typeof DOMMatrix === "function" && canvasPattern.setTransform) {
      var matrix6 = new DOMMatrix();
      matrix6.rotateSelf(0, 0, (pattern.rotation || 0) / Math.PI * 180);
      matrix6.scaleSelf(pattern.scaleX || 1, pattern.scaleY || 1);
      matrix6.translateSelf(pattern.x || 0, pattern.y || 0);
      canvasPattern.setTransform(matrix6);
    }
...
var GeoJSONRegion = function(_super) {
  ...
  GeoJSONRegion2.prototype.getBoundingRect = function() {
    ...
    for (; i < geometries.length; i++) {
      ...
      fromPoints(exterior, min23, max23);
      min(min3, min3, min23);
      max(max3, max3, max23);
    }
    if (i === 0) {
    ...
    return this._rect = new BoundingRect_default(min3[0], min3[1], max3[0] - min3[0], max3[1] - min3[1]);
  };
...
function estimateLabelUnionRect(axis) {
  ...
  for (var i = 0; i < tickCount; i += step2) {
...
***************************************************************************** */
//# sourceMappingURL=chunk-OSMGRWPM.js.map
