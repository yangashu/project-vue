<template>
  <div class="body">
      <el-row :gutter="24" class="row1">
          ...
          <el-input style="width:20%" class="input1" placeholder="请输入你要搜索的内容" v-model="b" @keyup.enter.native="selectLike()"></el-input>
      ...
      <el-row>
          <el-table ref="mt" :data="staffData" style="width:100%">
              <el-table-column prop="signId" label="员工编号">
			    <template #default="scope">{{scope.row.signId}}</template>
		      </el-table-column>
              <el-table-column prop="personalName" label="姓名"></el-table-column>
              <el-table-column prop="personalPhone" label="联系电话"></el-table-column>
              ...
              <el-table-column prop="signState" label="状态">
                  <template #default="scope">
                    ...
                    <div v-if="scope.row.signState==1" style="color: green;">已打卡</div>
                  </template>
              </el-table-column>
              <el-table-column label="操作">
                  <template #default="scope">
                    <el-button v-if="scope.row.signState==0" type="primary" @click="ab()">打卡</el-button>
                    <el-button v-if="scope.row.signState==1" type="danger" disabled>打卡</el-button>
                  </template>
      ...
      <el-dialog v-model="dialogVisible" title="识别人脸中。。。" center="true" width="30%" :before-close="stopNavigator">
...
export default {
  ...
  methods: {
      handleCurrentChange(page){
				...
				console.log(ps)
				this.axios.get("http://localhost:9090/satffsign/two",{params:this.pageInfo})
				.then(response =>{
					// console.log("1-------------------------------------------")
	...
	setImage() {
      ...
      fData.append('url',uu)
      this.axios.post("http://localhost:9090/Face/one",fData)
      .then(response=>{
    ...
    selectLike(){
        ...
        this.axios.get("http://localhost:9090/personal/one",{params:obj})
        .then(response =>{
            this.staffData=response.data.records;
		    this.pageInfo.total=response.data.total;
        })
