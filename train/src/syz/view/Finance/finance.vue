<template>
  <div>
    <div style="margin-bottom: 10px">
      ...
      >
        <el-table-column label="编号" prop="revenueId" />
        <el-table-column label="交易金额" prop="revenueMoney" />
        <el-table-column label="交易时间" prop="revenueTime" />
        <el-table-column label="交易类型">
          <template #default="scope">
            {{ scope.row.revenueType == 0 ? "收入" : "支出" }}
          </template>
        </el-table-column>
        <el-table-column label="支付方式" prop="revenueMode">
          <template #default="scope">
            {{
...
export default {
  ...
  methods: {
    ...
    // 导出
    exportrevenue() {
      request.get("/revenue/export").then((res) => {
        console.log(res);
      });
    },
    //   分页
    load() {
      request
        ...
        .then((res) => {
          if (res.data.code == "0") {
            this.revenueTableData = res.data.data.records;
...
//取消上传
    ...
    uploadFile() {
      if (this.fileList.length === 0) {
        ...
        // 请求自己服务器上传文件的接口
        request.post("/revenue/import",form,this.headers).then(res=> {
          if (res.code==="200"){
